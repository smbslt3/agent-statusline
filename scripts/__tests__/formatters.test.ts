...
describe('formatters', () => {
  ...
  describe('formatTimeRemaining', () => {
    ...
    it('should degrade to 0m for an unparseable timestamp (no "NaNm")', () => {
      expect(formatTimeRemaining('not-a-date', MOCK_TRANSLATIONS)).toBe('0m');
      expect(formatTimeRemaining(new Date('garbage'), MOCK_TRANSLATIONS)).toBe('0m');
    });
  });

  describe('formatResetClock', () => {
    it('should render same-day resets as HH:MM with no weekday', () => {
      const today = new Date();
      today.setHours(15, 30, 0, 0);
      expect(formatResetClock(today)).toBe('15:30');
    });

    it('should prefix the weekday for resets on a different day', () => {
      const otherDay = new Date(Date.now() + 3 * 24 * 60 * 60 * 1000);
      expect(formatResetClock(otherDay)).toMatch(/^(Sun|Mon|Tue|Wed|Thu|Fri|Sat) \d{2}:\d{2}$/);
    });

    it('should return the raw input for an unparseable timestamp', () => {
      expect(formatResetClock('whenever')).toBe('whenever');
    });
  });
  ...
  describe('formatResetDisplay', () => {
    ...
    it('remaining mode shows time left only', () => {
      expect(formatResetDisplay(future, 'remaining', MOCK_TRANSLATIONS)).toMatch(/^\d+h\d+m$/);
    });

    it('resetTime mode shows the clock only', () => {
      expect(formatResetDisplay(future, 'resetTime', MOCK_TRANSLATIONS)).toMatch(/^(?:[A-Z][a-z]{2} )?\d{2}:\d{2}$/);
    });

    it('both mode joins remaining and clock with a comma', () => {
      expect(formatResetDisplay(future, 'both', MOCK_TRANSLATIONS)).toMatch(/^\d+h\d+m, (?:[A-Z][a-z]{2} )?\d{2}:\d{2}$/);
    });
  });
  ...
  describe('shortenModelName', () => {
    ...
    it('should extract Fable from model name (dropping the version)', () => {
