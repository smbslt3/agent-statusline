...
/**
 * Build script for agent-statusline
 */
...
async function main() {
  try {
    ...
    // Build version-independent launcher entry point
    await build({
      ...commonOptions,
      entryPoints: ['scripts/launcher.ts'],
      outfile: 'dist/launcher.js',
    });

    console.log(`✓ Built dist/launcher.js with version ${version}`);
