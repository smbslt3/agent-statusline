...
const BUDGET_DIR = join(homedir(), '.cache', 'agent-statusline');
...
async function saveBudgetState(state: BudgetState): Promise<void> {
  try {
    if (!dirEnsured) {
      await mkdir(BUDGET_DIR, { recursive: true, mode: 0o700 });
      dirEnsured = true;
    }
    // Owner-only: the file records per-session spend and session ids, which must
    // not be world-readable on shared hosts. (POSIX mode bits; ignored on
    // Windows, which falls back to the owner-scoped per-user profile ACL.)
    await writeFile(BUDGET_FILE, JSON.stringify(state), { encoding: 'utf-8', mode: 0o600 });
    budgetCache = state;
  } catch (error) {
...
async function recordCostAndGetDailyImpl(
  ...
  // Constrain the session id to safe key characters before it indexes the
  // persisted map (defense against an unexpected id shape reaching the file).
