...
export const FILE_CACHE_DIR = process.env.AGENT_STATUSLINE_CACHE_DIR
  || path.join(os.homedir(), '.cache', 'agent-statusline');
 ...
 *
 * Files that don't match any prefix are left alone — they have their own
 * invalidation rules.
 */
 ...
 *
 * Note: the POSIX mode bits are owner-only on macOS/Linux; Node ignores them on
 * Windows, where the cache instead inherits the per-user profile ACL (already
 * owner-scoped on standard installs). The cache holds no secrets (usage
 * percentages, model names, slow git fields), so this is acceptable.
 */
...
export async function cleanupExpiredCache(cacheDir: string = FILE_CACHE_DIR): Promise<void> {
  ...
  try {
    ...
    // Advance the throttle only once the directory is readable, so a sweep that
    // failed because the cache dir didn't exist yet retries on the next save
