...
import { truncate, sanitizeText } from './formatters.js';
    ...
    if (entry.type === 'user' && entry.message?.content !== undefined) {
      ...
      if (typeof content === 'string') {
        ...
        for (const block of content) {
          ...
          if (m) {
            const name = m[1].trim();
            if (name.startsWith('/')) matchedName = sanitizeText(name);
            break;
          }
        }
      }
...
/**
 * In-flight parse coalescing: the transcript-consuming widgets (toolActivity,
 * todoProgress, agentStatus, slashCommand, sessionName) all call getTranscript()
 * concurrently within a single render's Promise.all. On the one-shot Claude host
 * the module cache is cold at the start of each process, so without coalescing
 * every widget would independently read + JSON.parse the entire transcript.jsonl
 * (up to ~5× per render). This collapses concurrent callers for the same path
 * onto a single parse, mirroring api-client's `pendingRequests`.
 */
let pendingParse: { path: string; promise: Promise<ParsedTranscript | null> } | null = null;

/**
 * Parse transcript JSONL file (in-flight deduped across concurrent callers).
...
export async function parseTranscript(
  ...
  try {
    ...
    if (pendingParse && pendingParse.promise === promise) pendingParse = null;
  }
}

async function parseTranscriptUncoalesced(
  transcriptPath: string
): Promise<ParsedTranscript | null> {
  try {
...
export function getRunningTools(
  ...
  for (const id of transcript.runningToolIds) {
    ...
    if (!tool) continue;
    // Tool name and target (file basename / Bash command / search pattern) are
    // attacker-influenceable (e.g. a hostile MCP tool or crafted path), so strip
    // control characters before they reach the rendered status line.
    ...
    running.push({
      ...
      startTime: tool.timestamp
        ...
        : Date.now(),
      target: target !== undefined ? sanitizeText(target) : undefined,
    });
  }
