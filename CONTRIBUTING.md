# Contributing to agent-statusline

Thank you for your interest in contributing to agent-statusline!
...
```bash
# Claude Code
claude --plugin-dir /path/to/agent-statusline

# Antigravity (agy) — install the built bundle, then restart agy
powershell -ExecutionPolicy Bypass -File .\install-agy.ps1   # macOS/Linux: sh ./install-agy.sh
```

The same `dist/index.js` auto-detects its host, so verify changes that touch the
shared render path on both Claude Code and agy when possible.
...
5. **Add any user-facing strings** to `locales/en.json` (the status line is English-only).
...
For deeper guidance on caching, theming, and error handling, study the reference widgets above — each is documented inline with the patterns it demonstrates.

## Pull Request Process

1. Run `npm test` and `npm run build`
2. **Commit the rebuilt `dist/` and synced `.claude-plugin/*.json`.** These artifacts are checked in (plugin users don't build), so CI fails any PR whose committed build is stale relative to the source.
3. Test your changes locally with Claude Code or agy
4. Update README.md / README.en.md if you've changed functionality
...
- Use GitHub Issues
- Include the host (Claude Code or agy) and its version
